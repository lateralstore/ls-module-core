<?
	...
	class LS_ConfigController extends Phpr_Controller
	{
		...
		protected function encryption_onSave()
		{
			try
			{
				...
				try
				{
					...
					$enc_test_salt = Db_ModuleParameters::get('core', 'enc_test_salt');
				} 
				catch (Exception $ex) {}
				
				if ($enc_test_value !== null)
				{
					if (Phpr_SecurityFramework::create()->salted_hash($enc_test_salt, $key) != $enc_test_value)
		...
		protected function restore_key_onRestore()
		{
			try
			{
				...
				Phpr::$session->flash['success'] = 'The encryption key for this '. Phpr::$config->get( 'APP_NAME', null ) .' installation is '.$config_content['config_key'];
				Phpr::$response->redirect($this->config_url());
			}
			catch (Exception $ex)
			{
