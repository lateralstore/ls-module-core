<?
	...
	class Core_UpdateManager
	{
		...
		protected function is_server_responsive($wait=5){
			...
			if (!strlen($uc_url))
				throw new Exception('Update Center server URL is not specified in the configuration file.');
		...
		protected function request_server_data($url, $fields = array(), $force = false, $time_limit=3600)
		{
			...
			if(Phpr::$config->get('FREEZE_LS_UPDATES')){
				throw new Exception("Updates from Update Center servers have been blocked by the system administrator.");
			}

			if(!$this->is_server_responsive()) {
				throw new Exception("Update servers are not responding.");
			}
		...
		public function request_ls_update_list( $hash = null, $force = false, $time_limit=15){
		...
		public function request_update_list($hash = null, $force = false)
		{
			...
			try {
				$response = $this->request_ls_update_list( $hash, $force, $time_limit = 10 );
			} catch(Exception $e){
				$response = array();
			}
		...
		public function update_application($cli_mode = false, $force = false)
		{
			...
			try {
				if ( !$force ) {
					$update_list = $this->request_ls_update_list();
		...
		public function get_updates_flag()
		{
			...
			try
			{
				...
				if (!$last_check)
				{
					try
					{
						$update_data = Core_UpdateManager::create()->request_ls_update_list(null,false,5);
		...
		{
			Core_Cli::print_line();
			Core_Cli::print_line('UPDATE TOOL');
			Core_Cli::print_line();
			...
			{
				Core_Cli::print_error('The application directory ('.PATH_APP.') is not writable for PHP.');
				exit(1);
			}
			...
			try
			{
				$update_data = $this->request_ls_update_list();
				...
				if (count($update_list))
				{
					...
					if (!Core_Cli::read_bool_option('Do you want to force update? (Y/N): '))
						exit(1);  
				}
					
				if (Core_EulaManager::pull())
				{
					Core_Cli::print_line("The End User License Agreement has changed. Please carefully read it. You must accept the new EULA before updating the Software.");
					Core_Cli::print_line();
				...
				Core_Cli::print_line('Updating Application...');
			...
			Core_Cli::print_line('Application has been successfully updated.');
			exit(0);
		}
		...
		public function set_license_info($data)
		{
			...
			if ($change_num >= self::allowed_license_change_num)
				throw new Phpr_ApplicationException('We are sorry, you cannot change the license details of this installation more than '.$max_change_num.' times');
			...
			$this->request_ls_update_list(md5($serial_number.$holder_name), true);
		...
		/**
		 * Triggered before update request is issued.
		...
		/**
		 * Triggered after update request is issued.
		...
		/**
		 * Triggered after software is updated.
