<?
	...
	class Core_EulaManager
	{
		public static function pull()
		{
			...
			if($url) {
				$request = Net_Request::create( $url );
		...
		{
			$url = Phpr::$config->get('EULA_GATEWAY', false);
			if(!$url){
				return false;
			}
			$eula_type = Phpr::$config->get('EULA_TYPE', 'std-eula');
			return $url.$eula_type.'/';
		}
		...
		public static function commit($login = null, $first_name = null, $last_name = null)
		{
			...
			try 
			{
				...
				Core_Email::sendToList('core', 'accepted_agreement', $viewData, 'Updated End User License Agreement has been accepted', $administrators);
			} catch (exception $ex) {}
		}
